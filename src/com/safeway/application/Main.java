package com.safeway.application;
...
import com.safeway.entities.Cliente;
import com.safeway.entities.Empresa;
import com.safeway.entities.Produto;
import com.safeway.entities.Usuario;
import com.safeway.entities.Venda;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		List<Produto> produtos = Arrays.asList(produto, produto2, produto3, produto4, produto5, produto6, produto7,produto8, produto9, produto10);
		...
		if (usuariosSearch.size() > 0) {
			...
			if ((usuarioLogado.getSenha().equals(senha))) {
				...
				if (usuarioLogado.IsEmpresa()) {
					...
					System.out.println("1 - SafeWay Padaria");
					System.out.println("2 - Level Varejo");
					System.out.println("3 - SafeWay Restaurante");
					Integer escolhaDaEmpresa = sc.nextInt();
					Empresa empresaEscolhida = empresas.stream().filter(x -> x.getId().equals(escolhaDaEmpresa)).collect(Collectors.toList()).get(0);//escolhe a empresa
					
					System.out.println();
					System.out.println("Empresa: "+empresaEscolhida.getNome() +" CNPJ: "+empresaEscolhida.getCnpj());
					System.out.println();
					System.out.println("1 - Listar vendas");
					System.out.println("2 - Ver produtos");
					System.out.println("0 - Deslogar");
					...
					case 1: {
						...
						System.out.println("VENDAS EFETUADAS");
						vendas.stream().forEach(venda -> {
							if (venda.getEmpresa().getId().equals(empresaEscolhida.getId())) {
								System.out.println("************************************************************");
								System.out.println("Venda de código: " + venda.getCódigo() + " no CPF "+ venda.getCliente().getCpf() + ": ");
								venda.getItens().stream().forEach(x -> {
									System.out.println(x.getId() + " - " + x.getNome() + "    R$" + x.getPreco());
					...
					case 2: {
						...
						System.out.println("MEUS PRODUTOS");
						produtos.stream().forEach(produto -> {
							if (produto.getEmpresa().getId().equals(empresaEscolhida.getId())) {
								System.out.println("************************************************************");
								System.out.println("Código: " + produto.getId());
								System.out.println("Produto: " + produto.getNome());
								System.out.println("Quantidade em estoque: " + produto.getQuantidade());
					...
					case 1: {
						...
						do {
							...
							Empresa empresaEscolhida = empresas.stream().filter(x -> x.getId().equals(escolhaEmpresa)).collect(Collectors.toList()).get(0);//escolhe a empresa
							
							for (Produto produtoSearch : produtos) {
								
								if (produtoSearch.getId().equals(escolhaProduto) && produtoSearch.getEmpresa().getId() == empresaEscolhida.getId()) {
								...
								if (productExist == false && escolhaProduto != 0) {
									System.out.println();
			...
			System.out.println();
			System.out.println("Dados Invalidos digite um numero corresponsente a lista");
			System.out.println();
			...
			executar(usuariosSearch, clientes, empresas, produtos, carrinho, vendas);
		}
	}
